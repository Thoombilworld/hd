...
(function(){
    ...
    tracks.forEach(track => {
        ...
        const dots = Array.from(track.querySelectorAll('[data-carousel-thumb]'));
        let pointer = 0;
        const go = (i) => {
            pointer = i;
            slides.forEach((s, idx) => s.classList.toggle('active', idx === pointer));
            dots.forEach((d, idx) => d.classList.toggle('active', idx === pointer));
        };
        dots.forEach((dot, idx) => {
            dot.addEventListener('click', () => go(idx));
        });
        setInterval(() => {
            go((pointer + 1) % slides.length);
        }, 6500);
    });
})();
</script>
